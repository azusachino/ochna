...
returning. `data.historical_snapshot` reports the temporary storage and elapsed
time; its `cleanup` value is `removed`. If a requested object is absent (as in
...
`historical_snapshot_unavailable` warning. `newly_unresolved_callers` records
call sites that became missing or ambiguous on the reviewed side. Base-to-
worktree mode warns when the current index is stale. Human output sections are
