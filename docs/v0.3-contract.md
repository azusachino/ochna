...
```json
{
  ...
  "schema": {"expected": 3, "actual": 3, "match": true},
...
with `next_action: "ochna sync"`. A dirty Git worktree is a warning, not a
failure, because `diff` must support uncommitted review work. `trust_verdict`
...
definitions or low-confidence resolution; low-quality locations are files or
packages whose unresolved/ambiguous rate crosses the documented threshold.
...
"min_confidence": 80, "nodes": [...], "edges": [...], "paths": [...],
...
confidence 80 or higher; `--min-confidence` is explicit, appears in output,
...
| `ochna_impact` | `impact --json` | `workspace`, `symbol`, optional `depth`, `direction`, `limit` |
